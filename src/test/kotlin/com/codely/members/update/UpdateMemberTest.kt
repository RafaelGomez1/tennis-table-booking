package com.codely.members.update
...
import com.codely.members.primaryadapter.rest.update.UpdateMemberController
import com.codely.members.primaryadapter.rest.update.UpdateMemberDTO
...
class UpdateMemberTest {
    ...
    fun `should update an existing member`() = runTest {
        ...
        val updated = repository.find(com.codely.members.domain.MemberFindByCriteria.ById(member.id))!!
        assertEquals("NewName", updated.name.value)
        assertEquals("NewSurname", updated.surname.value)
        assertEquals(Membership.Competition(com.codely.members.domain.Team.TWO_A), updated.type)
    }
